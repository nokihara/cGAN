...
def main():
    ...
    netG = Generator(nz=nz+40, nch_g=nch_g).to(device)   # 入力ベクトルの次元は、nz+クラス数7
    netG.apply(weights_init)
    print(netG)

    netD = Discriminator(nch=3+40, nch_d=nch_d).to(device)   # 入力Tensorのチャネル数は、nch+クラス数7
    ...
    writer = SummaryWriter(log_dir = "./All50e_logs")
    # グラフ作成用
    loss_num = 0
