...
def onehot_encode(label, device, n_class=3):  
    # ラベルをOne-Hoe形式に変換
    eye = torch.eye(n_class, device=device)
    # ランダムベクトルあるいは画像と連結するために(B, c_class, 1, 1)のTensorにして戻す
    return eye[label].view(-1, n_class, 1, 1)   

def concat_image_label(image, label, device, n_class=3):
    # 画像とラベルを連結する
                    ...
                    for j in range(9,13):
                        if j != 11:
                            ...
                            if line[j] == "-1":
                                k = 0
                            label.append(k)
                    #if line[9] =="1" or line[10] == "1" or line[12] == "1":
                    if line[9] == "1" and black <= 25000:
                        attr_list.append(label)
                        self.data.append(line[0])
                    if line[10] == "1" and blond <= 25000:
                        attr_list.append(label)
                        self.data.append(line[0])
                    if line[12] == "1" and brown <= 25000:
                        attr_list.append(label)
                        self.data.append(line[0])
                    #break
            #print("black", black)
...
def main():
    ...
    netG = Generator(nz=nz+3, nch_g=nch_g).to(device)   # 入力ベクトルの次元は、nz+クラス数7
    netG.apply(weights_init)
    print(netG)

    netD = Discriminator(nch=3+3, nch_d=nch_d).to(device)   # 入力Tensorのチャネル数は、nch+クラス数7
    ...
    fixed_label = [i for i in range(3)]   # 0〜6のラベルの繰り返し
    fixed_label = torch.tensor(fixed_label, dtype=torch.long, device=device)

    fixed_noise_label = concat_noise_label(fixed_noise, fixed_label, device)

    writer = SummaryWriter(log_dir = "./hair50e_25000logs")
    # グラフ作成用
    loss_num = 0
    ...
    for epoch in range(n_epoch):
        for itr, data in enumerate(dataloader):
            ...
            fake_label = torch.randint(3, (sample_size,), dtype=torch.long, device=device)  # 偽物画像のラベル
